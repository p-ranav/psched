...
struct TaskStats {
  ...
  template <typename T = std::chrono::milliseconds> long long wait_time() const {
  ...
  template <typename T = std::chrono::milliseconds> long long computation_time() const {
    return std::chrono::duration_cast<T>(end_time - start_time).count();
  }

  // wait_time() + computation_time()
  template <typename T = std::chrono::milliseconds> long long response_time() const {
